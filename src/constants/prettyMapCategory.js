const prettyMapCategory = {
  Entertainment: {
    color: "bg-purple-100 text-purple-800",
    hover: "hover:bg-purple-200 hover:text-purple-900",
    icon: "/icons/gamepad-2.svg"
  },
  Food: {
    color: "bg-yellow-100 text-yellow-800",
    hover: "hover:bg-yellow-200 hover:text-yellow-900",
    icon: "/icons/utensils.svg"
  },
  Healthcare: {
    color: "bg-red-100 text-red-800",
    hover: "hover:bg-red-200 hover:text-red-900",
    icon: "/icons/heart-pulse.svg"
  },
  Housing: {
    color: "bg-blue-100 text-blue-800",
    hover: "hover:bg-blue-200 hover:text-blue-900",
    icon: "/icons/house.svg"
  },
  Transportation: {
    color: "bg-green-100 text-green-800",
    hover: "hover:bg-green-200 hover:text-green-900",
    icon: "/icons/car.svg"
  },
  Utilities: {
    color: "bg-indigo-100 text-indigo-800",
    hover: "hover:bg-indigo-200 hover:text-indigo-900",
    icon: "/icons/wrench.svg"
  },
  Shopping: {
    color: "bg-pink-100 text-pink-800",
    hover: "hover:bg-pink-200 hover:text-pink-900",
    icon: "/icons/shirt.svg"
  },
  Income: {
    color: "bg-green-100 text-green-800",
    hover: "hover:bg-green-200 hover:text-green-900",
    icon: "/icons/piggy-bank.svg"
  },
  Transfer_in: {
    color: "bg-gray-100 text-gray-800",
    hover: "hover:bg-gray-200 hover:text-gray-900",
    icon: "/icons/circle-arrow-down.svg"
  },
  Transfer_out: {
    color: "bg-gray-100 text-gray-800",
    hover: "hover:bg-gray-200 hover:text-gray-900",
    icon: "/icons/circle-arrow-up.svg"
  },
  Bank_fees: {
    color: "bg-rose-100 text-rose-800",
    hover: "hover:bg-rose-200 hover:text-rose-900",
    icon: "/icons/receipt-text.svg"
  },
  Loans: {
    color: "bg-orange-100 text-orange-800",
    hover: "hover:bg-orange-200 hover:text-orange-900",
    icon: "/icons/hand-coins.svg"
  },
  Investments: {
    color: "bg-teal-100 text-teal-800",
    hover: "hover:bg-teal-200 hover:text-teal-900",
    icon: "/icons/chart-line.svg"
  },
  Personal_Care: {
    color: "bg-fuchsia-100 text-fuchsia-800",
    hover: "hover:bg-fuchsia-200 hover:text-fuchsia-900",
    icon: "/icons/sparkles.svg"
  },
  Subscription: {
    color: "bg-cyan-100 text-cyan-800",
    hover: "hover:bg-cyan-200 hover:text-cyan-900",
    icon: "/icons/bell-plus.svg"
  },
  Travel: {
    color: "bg-sky-100 text-sky-800",
    hover: "hover:bg-sky-200 hover:text-sky-900",
    icon: "/icons/plane.svg"
  },
  Other: {
    color: "bg-zinc-100 text-zinc-800",
    hover: "hover:bg-zinc-200 hover:text-zinc-900",
    icon: "/icons/badge-question-mark.svg"
  }
};


const prettyMapCategoryBgOnly = {
  Entertainment: { bg: "bg-purple-100" },
  Food: { bg: "bg-yellow-100" },
  Healthcare: { bg: "bg-red-100" },
  Housing: { bg: "bg-blue-100" },
  Transportation: { bg: "bg-green-100" },
  Utilities: { bg: "bg-indigo-100" },
  Shopping: { bg: "bg-pink-100" },
  Income: { bg: "bg-green-100" },
  Transfer_in: { bg: "bg-gray-100" },
  Transfer_out: { bg: "bg-gray-100" },
  Bank_fees: { bg: "bg-rose-100" },
  Loans: { bg: "bg-orange-100" },
  Investments: { bg: "bg-teal-100" },
  Personal_Care: { bg: "bg-fuchsia-100" },
  Subscription: { bg: "bg-cyan-100" },
  Travel: { bg: "bg-sky-100" },
  Other: { bg: "bg-zinc-100" }
};

export { prettyMapCategory, prettyMapCategoryBgOnly };
